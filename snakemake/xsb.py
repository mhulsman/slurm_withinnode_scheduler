...
def get_jobs():
    ...
    while attempt > 0 :
        try:
            jobs = s.list_jobs()
            return [int(jobid) for jobid, job_name, state, runtime, cpus, node in jobs]
...
def run(command, mem=1024, runtime='1:0:0', cpus=1):
    runtime = str(runtime)
    ...
    cwd = os.getcwd()
    env = dict([(str(a), str(b)) for a, b in os.environ.items()])
    ...
    print('xsbatch: error: Job submission failed, could not connect to XSlurm manager on ' + job_url)
